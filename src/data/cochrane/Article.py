...
class Article:
    ...
    def __init__(self, doi: str, url: str) -> None:
        self.doi = doi
        self.url = url
        self.content = {}
        #self.soup = soup
        #self.title = self.extract_title(soup=soup)
        #self.is_free = self.extract_is_free(soup=soup)
        #self.language_links = self.get_language_links(soup=soup)
